...
- **Your planning files** go in **`.planning/<date>-<slug>/` under your project directory**

| Location | What Goes There |
|----------|-----------------|
| Skill directory (`${CLAUDE_PLUGIN_ROOT}/`) | Templates, scripts, reference docs |
| `.planning/<date>-<slug>/` (in your project) | `task_plan.md`, `findings.md`, `progress.md` |

## Quick Start

Before ANY complex task, run `scripts/init-session.sh` (or `.ps1`) to scaffold the
three files under `.planning/<date>-<slug>/`, then:
...
> **Note:** Planning files go in `.planning/<date>-<slug>/` inside your project, not the skill installation folder.
...
- `scripts/init-session.sh` — Initialize planning files. Always creates an isolated plan under `.planning/YYYY-MM-DD-<slug>/`. With a name arg the slug derives from the name; without args it falls back to `untitled-<short>`. Default and parallel task workflows share the same structure.
...
- `scripts/resolve-plan-dir.sh` — Resolve the active plan directory. Checks `$PLAN_ID` env var first, then `.planning/.active_plan`, then newest plan dir by mtime, then falls back to a root-level `task_plan.md` if one is still present (read-side compatibility for plans created before this change). Used internally by hooks.
